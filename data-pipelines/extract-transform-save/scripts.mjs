...
const formTypes = [
  ...surveyForms,
  ...config.sources.googleForms.assignmentForms,
];
 ...
 * @param workshopCode
 */
export const saveWorkshopAttendancesAlongWithNonRegisteredUsers = (
  workshopCode
) => {
  const instructions = [];
  for (let day = 1; day <= 5; day++) {
    instructions.push(
      saveWorkshopDaySessionAttendanceAlongWithNonRegisteredUsers(
        workshopCode,
        day,
        'morning'
      )
...
const createUser = ({ _id, name }) => [
  `user is {_id: ${id}, name: ${name}}`,
  `index *user`,
];

const saveWorkshopDaySessionAttendanceAlongWithNonRegisteredUsers = (
  workshopCode,
  day,
  session
) => {
  return [
    ...
    [
      'if *raw_attendance.uniqueId = false, display *raw_attendance.',
      ...
      (ctx) => {
        ...
        userWorkshopData.connectTime[`day${day}`] = userWorkshopData
          .connectTime[`day${day}`] || {
          morning: 0,
          evening: 0,
        };
...
export const saveDifferentGoogleForms = (workshopCode) => {
  return formTypes.map((formType) =>
    googleFormImportInstructions(workshopCode, formType)
  );
};
...
const googleFormImportInstructions = (workshopCode, formType) => {
  return [
    ...
    [
      `if *loan_app matches {user._id: *formEntry.uniqueId, workshop._id: ${workshopCode}} as user-workshop.`,
      ...
      (ctx) => {
        ...
        if (surveyForms.includes(formType)) {
          //If is survey
          //Flag this form as filled
          const surveySubmissions =
            userWorkshopData.surveySubmissions ||
            (userWorkshopData.surveySubmissions = {});
          ...
          if (formType === postWorkshopSurvey) {
            ...
            const response =
              formEntry[
                'I am Interested to Participate in Higher Level of UHV workshops'
              ];
            //Mark whether user gave a non empty response to interest in higher level workshop
            if (
              response &&
              response.replace(config.regex.removeNonAlphabetNumeric, '')
                .length > 0
            ) {
          ...
          const assignmentSubmissions =
            userWorkshopData.assignments || (userWorkshopData.assignments = {});
